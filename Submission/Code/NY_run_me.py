...
data = pd.read_csv('../../Data/NY_dataset.csv', parse_dates=[0], index_col='DT',date_parser=dateparse)
X = data.values
...
plt.tight_layout()
plt.savefig("../Figures/NY/Correlation_matrix.png")
plt.clf()

#-----------------------------------------------------------------------
#Creating CSV with lag data for all features - to find optimal lag value (history)
#-----------------------------------------------------------------------
series = pd.Series.from_csv('../../Data/NY_dataset.csv',header=0)
df = pd.DataFrame()
for column in data:
    ...
    if column != "MSP":
        ...
        df.to_csv('../../Data/'+str(column)+'NY_Lagged_data.csv', index=False, sep=',')
    else:
        k =1
        for i in range(12,0,-1):
            df[str(column)+'t-'+str(i)]=data[column].shift(i)
    ...
    dataframe = pd.read_csv('../../Data/'+str(column)+'NY_Lagged_data.csv', header=0)
    ...
    plt.xlabel("# Lags")
    plt.ylabel("Rank")
    plt.savefig("../Figures/NY/"+str(column)+"Lag_data_rank.png")
    plt.clf()
...
df = df.dropna()
df.to_csv('../../Data/NY_Lagged_data.csv', index=False, sep=',')
...
#--------------------------------------------------------
# load data from new csv lag file
dataframe = pd.read_csv('../../Data/NY_Lagged_data.csv', header=0)
array = dataframe.values
# split into input and output
X_t = array[:,0:-2]
y_t = array[:,-2]
...
dataframe1.to_csv('../../Data/NY_New_Lagged_data.csv', index=False, sep=',')
...
plt.savefig("../Figures/NY/Predicted_MSP.png",bbox_inches='tight')
plt.tight_layout()
plt.clf()
...
plt.savefig("../Figures/NY/Forecasted_MSP.png",bbox_inches='tight')
plt.tight_layout()
plt.clf()
...
plt.savefig("../Figures/NY/Forecaste_RMSE.png",bbox_inches='tight')
plt.tight_layout()
plt.clf()
